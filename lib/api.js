 ...
 * @param {Object} response - Object containing spotify playlist's ID and a new access token if acquired.
 * @param {string} response.playlist_id - A spotify playlist's ID.
 ...
 * @param {playlistCallback} callback - Calls the callback with the playlist id and possibly a new access token.
 */
 ...
 *
 * @param {function} generator_function - The function to call to generate a playlist.
 * @param {array} words - The words to be converted into a playlist.
 * @param {Object} credentials - The user's access information for spotify.
 * @param {string} credentials.access_token - The user's access token for spotify.
 * @param {string} credentials.refresh_token - The user's refresh token for spotify.
 * @param {playlistCallback} callback - Calls the callback with the playlist id and possibly a new access token.
 */
function createPlaylist(generator_function, words, credentials, callback) {
  ...
  spotify.getMe(access_token, function(data) {
    ...
    if (user_id === undefined) { // token has expired
      spotify.requestToken({grant_type: 'refresh_token', refresh_token : refresh_token}, function(new_credentials) {
        spotify.getMe(new_credentials.access_token, function(data) {
          var user_id = data.id;
          generator_function(words, new_credentials.access_token, user_id, function(playlist_id) {
            var response = {
              user_id : user_id,
      ...
      generator_function(words, access_token, user_id, function(playlist_id) {
        var response = {
          user_id : user_id,
          playlist_id : playlist_id
        }
